...
from backend.app.scanners import bandit, gitleaks, semgrep, trivy


async def run(state: AuditState) -> dict[str, object]:
    await publish_agent_state(state["task_id"], "StaticScan", "running", "正在执行 Semgrep、Bandit、Gitleaks、Trivy 扫描", 60)
    ...
    message = f"静态扫描完成，发现 {len(results)} 条结果"
